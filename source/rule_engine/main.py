...
# CODA SPECIFICA PER IL RULE ENGINE
...
def process_event(event):
    ...
    try:
        ...
        for op, thresh, act, act_val in cur.fetchall():
            if check_condition(val, op, thresh):
                print(f"[RuleEngine] MATCH: {sid} {op} {thresh} -> {act}={act_val}")
                requests.post(f"{SIMULATOR_URL}/{act}", json={"state": act_val})
...
def main():
    ...
    while True:
        try:
            ...
            print("[RuleEngine] Listening on Fanout...")
            ch.start_consuming()
        except: time.sleep(5)
